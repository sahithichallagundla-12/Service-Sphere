...
const navItems = [
  ...
  { href: "/vendor/issues", label: "Issues Board", icon: Kanban },
  { href: "/vendor/performance", label: "Performance", icon: BarChart3 },
  { href: "/vendor/penalties", label: "Penalties", icon: AlertTriangle },
  ...
  return (
    <aside className="flex h-full w-64 flex-col bg-sidebar text-sidebar-foreground">
      ...
      <div className="flex h-16 items-center gap-2 px-6 border-b border-sidebar-border">
        ...
        <span className="ml-auto rounded bg-sidebar-accent px-2 py-0.5 text-xs">Vendor</span>
      </div>
            ...
            <Button variant="ghost" className="w-full justify-start gap-2 text-sidebar-foreground hover:bg-sidebar-accent">
              <Avatar className="h-8 w-8">
                <AvatarFallback className="bg-sidebar-primary text-sidebar-primary-foreground text-xs">
                  {profile?.company_name?.slice(0, 2).toUpperCase() || "V"}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 text-left">
                <p className="text-sm font-medium truncate">{profile?.company_name || "Vendor"}</p>
